App = {
  ...
  initContract: async function() {
    $.getJSON('CryptoRevolution.json', function(data) {
      ...
      App.contracts.CryptoRevolution = new web3.eth.Contract(CryptoRevolutionArtifact, App.contract_address);
  ...
  ConvertNFT: async function(){
    ...
    const gasAmount = await App.contracts.CryptoRevolution.methods.convertNFT(App.account, nft_level).estimateGas({ from: App.account });
    console.log("Convert gas - " + gasAmount);
    return App.contracts.CryptoRevolution.methods.convertNFT(App.account, nft_level).send({from: App.account, gas: gasAmount})
  ...
  TransferNFT: async function() {
    ...
    const gasAmount = await App.contracts.CryptoRevolution.methods.safeTransferFrom(App.account, to_address, parseInt(nft_level), 1, "0x5174e853").estimateGas({ from: App.account, to: to_address });
    //console.log("Mint gas - " + gasAmount);
    await App.contracts.CryptoRevolution.methods.safeTransferFrom(App.account, to_address, parseInt(nft_level), 1, "0x5174e853").send({from: App.account, to: to_address, gas: gasAmount})
    .on('transactionHash', function(hash){
    })
    .on('receipt', function(receipt){
    })
